...
function loadApiKey() {
  ...
  throw new Error('OPENAI_API_KEY not found. Export it or add to .env');
}
...
function buildTasks(story) {
  ...
  story.chapters.forEach((chapter) => {
    ...
    chapter.scenes.forEach((scene, sceneIdx) => {
      ...
      const voiceId = `ch${chapterId}_scene${sceneIdx}`;
      tasks.set(voiceId, text);
      ...
      if (scene.learning) {
        ...
        tasks.set(learningVoiceId, cleanText(scene.learning.content || scene.text));
      }
    });
...
async function synthesize(apiKey, text) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `OpenAI TTS failed (${response.status} ${response.statusText}): ${errText}`
    );
  }
...
async function main() {
  ...
  console.log(
    `\nAll ${tasks.size} voice files regenerated with voice '${OPENAI_VOICE}'.`
  );
}
